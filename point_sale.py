...
class pos_session(osv.osv):
    ...
    def wkf_action_close(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            for st in record.statement_ids:
                if abs(st.difference) > st.journal_id.amount_authorized_diff:
                    ...
                    if not self.pool.get('ir.model.access').check_groups(cr, uid, "point_sale.group_pos_manager"):
                        raise osv.except_osv( _('Error!'),
        ...
        obj = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'point_sale', 'menu_point_root')[1]
        return {
            'type' : 'ir.actions.client',
...
class product_product(osv.osv):
    ...
    def _default_pos_categ_id(self, cr, uid, context=None):
        ...
        try:
            category_id = proxy.get_object_reference(cr, uid, 'point_sale', 'categ_others')[1]
        except ValueError:
            values = {
                'name' : 'Others',
            }
